...
#/***************************************************************************#
#Filename           :      BECOME simulation.Rmd
#Author             :      obrieea1
#Date               :      xx-xx-2017
#R                  :      3.2.3 (2015-12-10) 
#Platform           :      x86_64-pc-linux-gnu (64-bit) 
#Project/Study      :      xxxxxxxxxxxxxxx [example]
#Description        :      Attempt at simulating and analysing BECOME trial 
#Assumptions:   
#Input              :         
#Output             :      
#Macros used        :      <macro>
...
cat("\nAttempt at simulation and analysis of the BECOME trial. I simulate between country and between site SD with an underlying mean (see reference whre code was helpful).\n")
...
```{r, echo=FALSE, eval=FALSE}
    ...
    foo <-  sasxport.get("portable.xpt")      # Erik created XPORT files 15MAY2017
    adcu <-  sasxport.get("adcu.xpt")         # needed to identify CSU with concomitant CINDU  
    setwd(wd.code)  
...
```{r, echo=TRUE, eval=FALSE}
    ...
    # save the workspace
    save.image(file=paste(wd.data,"\\brms BECOME bernouli model.RData", sep=""))
...
```{r , eval=TRUE}
    ...
    load(file=paste(wd.data,"\\brms BECOME bernouli model.RData", sep=""))  #site sd=2.5, country=0.5
   # load(file=paste(wd.data,"\\brms BECOME bernouli model1.RData", sep="")) #sitesd=countrysd=0.5
    library(brms)
...
```{r, echo=TRUE ,eval=TRUE, fig.width=7, fig.height=12} 
 ...
 # start here use coefficients from model rather than predictions
    ...
    e2 <- A(fixef(fit)[,'2.5%ile'])
    e3 <- A(fixef(fit)[,'97.5%ile'])
   ...
   # df <- df[order(sites),]
    ...
    fp <- NULL
    fp <- ggplot(data=df, aes(x=label, y=Estimate, ymin=X2.5.ile, ymax=X97.5.ile, colour=label)) +
      geom_pointrange() +
         geom_hline(yintercept=mu, color="green",lty=2) +  # add a dotted line 
      
      geom_hline(yintercept=e1,lty=2, color="blue", linetype="dashed") +  # estimate
      geom_hline(yintercept=e2,  lty=2, color="blue", linetype="dashed") +   
      geom_hline(yintercept=e3, lty=2,  color="blue", linetype="dashed") +   
...
```{r, eval=TRUE, fig.width=7, fig.height=12} 
  ...
  fp <- ggplot(data=df, aes(x=label, y=mean, ymin=lower, ymax=upper, colour=co)) +
          geom_pointrange() + 
          geom_hline(yintercept=mu, lty=2) +  # add a dotted line at x=1 after flip
          geom_hline(yintercept= A(fix), lty=2, color="blue", linetype="dashed") +  # estimate
          geom_hline(yintercept=A(L), lty=2, color="blue", linetype="dashed") +   
          geom_hline(yintercept=A(U), lty=2, color="blue", linetype="dashed") +   
          coord_flip() +  # flip coordinates (puts labels on y axis)
          xlab("Label") + ylab("Mean (95% CI)") +
          theme_bw() + # use a white background +
    theme(legend.position="none") 
